...
export const analyzeLanguages = (repos) => {
  ...
  const languages = Object.values(langStats)
    .map((lang) => ({
      name: lang.name,
      count: lang.count,
      percentage: totalWeight > 0 ? Math.round((lang.weight / totalWeight) * 100) : 0,
    }))
...
export const analyzeActivity = (repos) => {
  ...
  if (diffDaysFromLatest <= 7) {
    ...
    description = 'Pushed to a repository within the past week. Active contributions observed.';
    score = 95;
  } else if (diffDaysFromLatest <= 30) {
    status = 'Steady';
    description = 'Pushed to a repository in the past month. Consistent maintenance.';
    score = 80;
  } else if (diffDaysFromLatest <= 90) {
    status = 'Occasional';
    description = 'Pushed within the last 3 months. Semi-regular contributions.';
    score = 50;
  }
...
export const getComplexityLabel = (score) => {
  if (score >= 70) return { label: 'High Complexity (Production)', class: 'complexity-high' };
  if (score >= 40) return { label: 'Medium Complexity (Utility)', class: 'complexity-medium' };
...
export const evaluateReadmeQuality = (readmeText) => {
  ...
  if (length > 2500) {
    ...
    score += 20;
    suggestions.push('Expand your README with more detailed descriptions or usage examples.');
  } else if (length > 300) {
    score += 10;
    suggestions.push('Your README is brief. Consider adding installation guides, features list, and context.');
  } else {
    score += 2;
    suggestions.push('Extremely short README. Add basic documentation to describe what the code does.');
  }

  // Check section titles (regex matches headers)
  const hasInstallation = /#+\s*(installation|getting\s+started|setup|install)/i.test(readmeText);
  const hasUsage = /#+\s*(usage|how\s+to\s+use|examples|quick\s*start)/i.test(readmeText);
  const hasLicense = /#+\s*(license|licence)/i.test(readmeText);
  const hasContributing = /#+\s*(contributing|contribute)/i.test(readmeText);
  const hasFeatures = /#+\s*(features|functionality|what\s+it\s+does)/i.test(readmeText);
  
  // Section checks (Max 40)
  if (hasInstallation) {
    score += 10;
  } else {
    suggestions.push('Add an "Installation" section to show how to install dependencies and run the project.');
  }

  if (hasUsage) {
    score += 10;
  } else {
    suggestions.push('Add a "Usage" section with code blocks or CLI commands showing how to use the app.');
  }
  ...
  if (hasLicense) {
    ...
    suggestions.push('Add a "License" section to clarify open source rights (e.g. MIT, Apache).');
  }
  ...
  if (codeBlocksCount >= 2) {
    ...
    score += 8;
    suggestions.push('Consider adding more code snippets demonstrating core function calls or configurations.');
  } else {
    suggestions.push('Add code blocks (using markdown ``` syntax) to show syntax-highlighted code or terminal commands.');
  }

  if (hasImages || hasBadges) {
    score += 15;
  } else {
    suggestions.push('Add visual elements like screenshots, GIFs, or shields/badges to make the README engaging.');
  }
...
export const runPortfolioAnalysis = (profile, repos) => {
  ...
  const avgComplexity = topRepos.length > 0
    ? Math.round(topRepos.reduce((acc, r) => acc + r.complexity, 0) / topRepos.length)
    : 0;
