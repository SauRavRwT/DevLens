...
`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Gemini API error: ${response.status} ${response.statusText}. Check your API Key.`);
    }
    ...
    if (!report.strengths || !report.weaknesses || !report.resumeBulletPoints || !report.suggestedCareerPaths || !report.missingSkills) {
      throw new Error('AI returned an incomplete data structure.');
    }
    
    return report;
  } catch (error) {
    console.error('Failed to generate AI report, falling back to local engine:', error);
...
export const generateLocalReport = (profile, analysis) => {
  ...
  const strengths = [
    ...
    analysis.activity.status === 'Highly Active' || analysis.activity.status === 'Steady'
  ...
  if (lowercaseLangs.includes('javascript') || lowercaseLangs.includes('typescript') || lowercaseLangs.includes('html') || lowercaseLangs.includes('css')) {
  ...
  } else if (lowercaseLangs.includes('python') || lowercaseLangs.includes('r') || lowercaseLangs.includes('julia')) {
    suggestedCareerPaths = [
      {
  ...
  // 5. MISSING SKILLS
  let missingSkills;
  if (lowercaseLangs.includes('javascript') || lowercaseLangs.includes('typescript')) {
    missingSkills = [
