...
export const fetchGitHubProfile = async (username, token = "") => {
  ...
  if (response.status === 403 || response.status === 429) {
    throw new Error(
      "GitHub API rate limit exceeded. Please provide a GitHub Token in settings to raise limits.",
    );
  }
...
export const fetchGitHubRepos = async (username, token = "") => {
  ...
  if (!response.ok) {
    if (response.status === 403 || response.status === 429) {
      throw new Error(
        "GitHub API rate limit exceeded. Please configure a GitHub Token in settings.",
      );
    }
